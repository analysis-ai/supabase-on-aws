...
export class SupabaseStack extends FargateStack {
  ...
  constructor(scope: Construct, id: string, props: cdk.StackProps = {}) {
    ...
    const jwtExpiryLimit = new cdk.CfnParameter(this, 'JwtExpiryLimit', {
      description:
        'How long tokens are valid for. Defaults to 3600 (1 hour), maximum 604,800 seconds (one week).',
      type: 'Number',
      default: 3600,
      minValue: 300,
      maxValue: 604800
    ...
    const senderEmail = new cdk.CfnParameter(this, 'Email', {
      description:
        'This is the email address the emails are sent from. If Amazon WorkMail is enabled, it set "noreply@supabase-<account_id>.awsapps.com"',
      type: 'String',
      default: 'help@anny.ai',
      allowedPattern:
    ...
    const authImageUri = new cdk.CfnParameter(this, 'AuthImageUri', {
      type: 'String',
      default: 'public.ecr.aws/supabase/gotrue:v2.151.0',
      description: 'https://gallery.ecr.aws/supabase/gotrue'
    });
    const restImageUri = new cdk.CfnParameter(this, 'RestImageUri', {
      type: 'String',
      default: 'public.ecr.aws/supabase/postgrest:v12.2.0',
      description: 'https://gallery.ecr.aws/supabase/postgrest'
    });
    const kongImageUri = new cdk.CfnParameter(this, 'KongImageUri', {
      type: 'String',
      default: 'public.ecr.aws/k1e4k0b3/kong:latest',
      description: 'https://gallery.ecr.aws/k1e4k0b3/kong'
    });
    const realtimeImageUri = new cdk.CfnParameter(this, 'RealtimeImageUri', {
      type: 'String',
      default: 'public.ecr.aws/supabase/realtime:v2.29.15',
      description: 'https://gallery.ecr.aws/supabase/realtime'
    });
    const storageImageUri = new cdk.CfnParameter(this, 'StorageImageUri', {
      type: 'String',
      default: 'public.ecr.aws/supabase/storage-api:v1.6.7',
    ...
    const postgresMetaImageUri = new cdk.CfnParameter(
      this,
      'PostgresMetaImageUri',
      {
        type: 'String',
        default: 'public.ecr.aws/supabase/postgres-meta:v0.83.1',
        description: 'https://gallery.ecr.aws/supabase/postgres-meta'
      }
    );

    /** The flag for High Availability */
    const enableHighAvailability = new cdk.CfnParameter(
      this,
    ...
    const highAvailability = new cdk.CfnCondition(this, 'HighAvailability', {
      expression: cdk.Fn.conditionEquals(enableHighAvailability, 'true')
    });
    ...
    const webAclArn = new cdk.CfnParameter(this, 'WebAclArn', {
      ...
      default: '',
      allowedPattern:
        '^arn:aws:wafv2:us-east-1:[0-9]{12}:global/webacl/[\\w-]+/[\\w]{8}-[\\w]{4}-[\\w]{4}-[\\w]{4}-[\\w]{12}$|'
    });

    /** The minimum number of aurora capacity units */
    const minACU = new cdk.CfnParameter(this, 'MinACU', {
      description:
        'The minimum number of Aurora capacity units (ACU) for a DB instance in an Aurora Serverless v2 cluster.',
      type: 'Number',
      default: 0.5,
      minValue: 0.5,
      maxValue: 128
    ...
    const maxACU = new cdk.CfnParameter(this, 'MaxACU', {
      description:
        'The maximum number of Aurora capacity units (ACU) for a DB instance in an Aurora Serverless v2 cluster.',
      type: 'Number',
      default: 16,
      minValue: 0.5,
      maxValue: 128
    ...
    const sesRegion = new cdk.CfnParameter(this, 'SesRegion', {
      description:
        'Amazon SES used for SMTP server. If you want to use Amazon WorkMail, need to set us-east-1, us-west-2 or eu-west-1.',
      type: 'String',
      default: 'us-west-2',
      allowedValues: [
        'us-east-1',
    ...
    const enableWorkMail = new cdk.CfnParameter(this, 'EnableWorkMail', {
      description:
        'Enable test e-mail domain "xxx.awsapps.com" with Amazon WorkMail.',
      type: 'String',
      default: 'false',
      allowedValues: ['true', 'false']
    });
    /** CFn condition for Amazon WorkMail */
    const workMailEnabled = new cdk.CfnCondition(this, 'WorkMailEnabled', {
      expression: cdk.Fn.conditionEquals(enableWorkMail, 'true')
    });
    ...
    new cdk.CfnRule(this, 'CheckWorkMailRegion', {
      ...
      assertions: [
        {
          assert: cdk.Fn.conditionContains(
            ['us-east-1', 'us-west-2', 'eu-west-1'],
            sesRegion.valueAsString
          ),
          assertDescription:
            'Amazon WorkMail is supported only in us-east-1, us-west-2 or eu-west-1. Please change Amazon SES Region.'
        }
      ]
    });
    ...
    // Overwrite ACU
    (
      db.cluster.node.defaultChild as rds.CfnDBCluster
    ...
    const supabaseStorageAdminSecret = db.genUserPassword(
     ...
     */
    const anonKey = jwtSecret.genApiKey('AnonKey', {
      roleName: 'anon',
      issuer: 'supabase',
      expiresIn: '10y'
    });
     ...
     */
    const serviceRoleKey = jwtSecret.genApiKey('ServiceRoleKey', {
    ...
    const kong = new AutoScalingFargateService(this, 'Kong', {
      ...
      taskImageOptions: {
        ...
        image: ecs.ContainerImage.fromRegistry(kongImageUri.valueAsString), // FIX: new kong-template.yml version
        ...
        environment: {
          ...
          KONG_PLUGINS:
            'request-transformer,cors,key-auth,acl,basic-auth,opentelemetry',
    ...
    const auth = new AutoScalingFargateService(this, 'Auth', {
      ...
      taskImageOptions: {
        ...
        secrets: {
          GOTRUE_DB_DATABASE_URL: ecs.Secret.fromSecretsManager(
            supabaseAuthAdminSecret,
            'uri'
          ),
          GOTRUE_JWT_SECRET: ecs.Secret.fromSecretsManager(jwtSecret),
          GOTRUE_SMTP_USER: ecs.Secret.fromSecretsManager(
            smtp.secret,
            'username'
          ),
          GOTRUE_SMTP_PASS: ecs.Secret.fromSecretsManager(
            smtp.secret,
            'password'
          )
        }
    ...
    const authProviders = auth.addExternalAuthProviders(
      `${apiExternalUrl}/auth/v1/callback`,
      3
    );
    ...
    const meta = new AutoScalingFargateService(this, 'Meta', {
      ...
      taskImageOptions: {
        image: ecs.ContainerImage.fromRegistry(
          postgresMetaImageUri.valueAsString
        ),
        containerPort: 8080,
        environment: {
          PG_META_PORT: '8080',
        ...
        secrets: {
          PG_META_DB_NAME: ecs.Secret.fromSecretsManager(
            supabaseAdminSecret,
            'dbname'
          ),
          PG_META_DB_USER: ecs.Secret.fromSecretsManager(
            supabaseAdminSecret,
            'username'
          ),
          PG_META_DB_PASSWORD: ecs.Secret.fromSecretsManager(
            supabaseAdminSecret,
            'password'
          )
        }
    ...
    kong.service.taskDefinition.defaultContainer!.addEnvironment(
      'SUPABASE_AUTH_URL',
      `${auth.endpoint}/`
    );
    kong.service.taskDefinition.defaultContainer!.addEnvironment(
    ...
    const forceDeployJob = new ForceDeployJob(this, 'ForceDeployJob', {
      cluster
    });
    ...
    const cfnInterface = {
      ParameterGroups: [
        ...
        },
        {
          ...
          Parameters: [enableHighAvailability.logicalId, webAclArn.logicalId]
        },
        {
          Label: { default: 'Infrastructure Settings - Database' },
          Parameters: [minACU.logicalId, maxACU.logicalId]
        },
        {
    ...
    for (let i = 0; i < authProviders.length; i++) {
      ...
      cfnInterface.ParameterLabels[provider.name.logicalId] = {
        default: 'Provider Name'
      };
      cfnInterface.ParameterLabels[provider.clientId.logicalId] = {
        default: 'Client ID'
      };
      cfnInterface.ParameterLabels[provider.secret.logicalId] = {
